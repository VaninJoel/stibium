'''Parsing Antimony code and recovering from syntax errors.
'''
...
def get_grammar_str():
    '''Load the grammar string from `antimony.lark`'''
...
def dummy_newline() -> Token:
    '''Helper that creates a dummy/fake newline Lark Token'''
    return Token('NEWLINE', DUMMY_VALUE, 0, 1, 1, 1, 1, 0)  # type: ignore


def is_dummy(node: Union[Token, Tree]):
    '''Helper that tests whether a given node is a dummy.'''
    return isinstance(node, Token) and node.value == DUMMY_VALUE


def get_parser(grammar_str: str, cache_file: str):
    '''Helper that creates the Lark parser object.
    
    Args:
        grammar_str: The string for Antimony grammar
        cache_file: The parser cache file path.
    '''
...
class AntimonyParser:
    ...
    def parse(self, text: str, recoverable=False) -> FileNode:
        ...
        # convert from Lark tree to our tree
        root = transform_tree(tree)
        assert root is not None and isinstance(root, FileNode)

        # set the parent pointers for all nodes in the tree
        set_parents(root)
        # set the next and prev pointers for all leaves in the tree
        set_leaf_pointers(root)
        return root

    def _parse_with_puppet(self, puppet, recoverable: bool, text: str, token_callback = None):
        '''Parse with the given puppet, optionally in a recoverable way.

        Args:
            puppet: Lark's ParserPuppet (or InteractiveParser in newer releases) object
            recoverable: Whether an error should be thrown when a syntax error is encountered, or
                         try and recover. In the latter case, ErrorTokens and ErrorNodes are
                         inserted at the corresponding places in the tree.
            text: The source text. This is only necessary for generating tokens.
            token_callback: Called whenever a token is lexed (or generated by the error recovery
                            system), with that token as the argument.
        '''
        ...
        while True:
            try:
                # Main parser loop. This replicates the same loop in Lark.
    ...
    def _recover_from_error(self, err_puppet, token_callback, token=None):
        ...
        def recover_stacks(value_stack, state_stack, start_index):
            ...
            if all_nodes:
                ...
                if not (len(all_nodes) == 1 and all_nodes[0] and is_dummy(all_nodes[0])):
                    # propgate node ranges up the tree
                    meta = Meta()
                ...
                # backtrack the state and value stacks since the previous nodes/tokens cannot be
                # combined to form a valid tree, due to the unexpecetd token.
        ...
        manual_add = False
        if token:
            # If the error token exists (i.e. the lexer was able to lex a full token but the
            # token was unexpected), then simply change its type.
            token.type = 'ERROR_TOKEN'
        else:
            # The error token may not exist if the lexer failed to match any token using its
            # available rules (e.g. it encountered something not part of the grammar, like
            # '?' or '~'). In this case no error token is provided by Lark, and we need to
            # manually create an ErrorToken that is 1-character long.
            #
            # In other words, in case Lark fails to lex, we do a "dumb" lex with one character
            # after the cursor.
